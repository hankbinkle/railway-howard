...
async function trelloGet(path) {
  ...
  if (!res.ok) throw new Error(`Trello GET ${path} failed: ${res.status} ${res.statusText}`);
  return res.json();
}
...
async function getCardsInList(listId) {
  return trelloGet(`/lists/${listId}/cards`);
}
 ...
 */
async function addCheckitem(cardId, text, memberId) {
  ...
  await trelloPost(`/checklists/${checklistId}/checkItems`, body);
  console.log(`[shared/trello] Checkitem added to ${cardId}: "${text}"`);
}
 ...
 * Finds lines starting with "email:" or "Email:".
