...
async function getAccessToken() {
  try {
    ...
    if (!res.ok) {
      console.error(`[shared/dropbox] OAuth refresh failed: ${data.error_description || data.error}`);
      return null;
    }
...
async function getTeamMemberId(token) {
  ...
  try {
    const res = await fetch(`${DROPBOX_API}/2/team/members/get_info`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify([{ '.tag': 'email', email: DROPBOX_TEAM_MEMBER_EMAIL }]),
    });
    ...
    const memberId = data.profile?.team_member_id || data.team_member_id;
    if (memberId) {
 ...
 * @param {string} token - Valid Dropbox access token
 * @param {string} memberId - Team member ID for Select-User
 * @returns {Promise<string|null>}
 */
async function getTeamRootNamespace(token, memberId) {
  try {
    const res = await fetch(`${DROPBOX_API}/2/users/get_current_account`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (!res.ok) {
      console.error(`[shared/dropbox] get_current_account failed: ${res.status}`);
      return null;
    }

    const data = await res.json();
    const ns = data.root_info?.root_namespace_id;
    if (ns) {
      console.log(`[shared/dropbox] Team root namespace: ${ns}`);
      return ns;
    }
    return null;
  } catch (e) {
...
async function uploadFile(fileBuffer, dropboxPath, options = {}) {
  ...
  try {
    const res = await fetch(`${DROPBOX_CONTENT}/2/files/upload`, {
      method: 'POST',
      headers,
      body: fileBuffer,
    });
...
async function createSharedLink(dropboxPath, token, memberId, rootNs) {
  try {
    ...
    const linkRes = await fetch(`${DROPBOX_API}/2/sharing/create_shared_link_with_settings`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        path: dropboxPath,
...
async function copyFolder(sourcePath, destPath) {
  ...
  try {
    const res = await fetch(`${DROPBOX_API}/2/files/copy_v2`, {
      method: 'POST',
      headers,
